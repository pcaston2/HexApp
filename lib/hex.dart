...
Map <EdgeDirection, Point> get edge => {
  EdgeDirection.NorthEast: (vertex[VertexDirection.East] + vertex[VertexDirection.NorthEast]) / 2.0,
  EdgeDirection.North: (vertex[VertexDirection.NorthWest] + vertex[VertexDirection.NorthEast]) / 2.0,
  EdgeDirection.NorthWest: (vertex[VertexDirection.West] + vertex[VertexDirection.NorthWest]) / 2.0,
  EdgeDirection.SouthWest: (vertex[VertexDirection.SouthWest] + vertex[VertexDirection.West]) / 2.0,
  EdgeDirection.South: (vertex[VertexDirection.SouthEast] + vertex[VertexDirection.SouthWest]) / 2.0,
  EdgeDirection.SouthEast: (vertex[VertexDirection.East] + vertex[VertexDirection.SouthEast]) / 2.0
};
...
class Hex {
  int q = 0;
  int r = 0;
  get point => new Point( (3.0*width/4.0) * q, height * r + height/2.0 * q );
  ...
  static Hex GetHexPartFromPoint(Point p) {
    ...
    if (offset.magnitude < size / 3) {
      ...
      var closestEdgeDirection = edge.entries.firstWhere((e) => e.value == closestEdge).key;
      ...
      if ((offset-closestEdge).magnitude < (offset-closestVertex).magnitude) {
        ...
        switch (closestEdgeDirection) {
          ...
          case EdgeDirection.SouthEast:
            //return currHex;
            return new Edge(EdgeType.West, currHex.q + hexOffset.q, currHex.r + hexOffset.r);
            break;
          case EdgeDirection.South:
            //return currHex;
            return new Edge(EdgeType.North, currHex.q + hexOffset.q, currHex.r + hexOffset.r);
            break;
          case EdgeDirection.SouthWest:
            //return currHex;
            return new Edge(EdgeType.East, currHex.q + hexOffset.q, currHex.r + hexOffset.r);
            break;
        }
      } else {
        var closestVertexDirection = vertex.entries.firstWhere((e) => e.value == closestVertex).key;
...
class Face extends Hex {

  Face(q, r) : super.position(q,r);
  Face.origin() : super.origin();
